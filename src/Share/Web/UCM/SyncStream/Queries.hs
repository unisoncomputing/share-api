...
allSerializedDependenciesOfCausalCursor :: CausalId -> CodebaseM e (PGCursor ByteString)
    ...
    [sql|
        ...
        transitive_components(component_hash_id) AS (
            ...
            FROM all_patches ap
             ...
             FROM copied_causals cc
               JOIN serialized_causals sc ON cc.causal_id = sc.causal_id
               JOIN bytes ON sc.bytes_id = bytes.id
           UNION ALL
           SELECT bytes.bytes
             FROM copied_namespaces cn
               JOIN serialized_namespaces sn ON cn.namespace_hash_id = sn.namespace_hash_id
               JOIN bytes ON sn.bytes_id = bytes.id
           UNION ALL
           SELECT bytes.bytes
             FROM copied_patches cp
               JOIN serialized_patches sp ON cp.patch_id = sp.patch_id
