module Enlil.Postgres.Patches.Queries
  ( expectPatch,
    savePatch,
  )
where
...
import Enlil.Codebase.Types (CodebaseEnv (codebaseOwner), CodebaseM)
import Enlil.Postgres
import Enlil.Postgres.Definitions.Queries qualified as Defn
import Enlil.Postgres.Definitions.Types (PgComponentIndex, pgComponentIndex, unPgComponentIndex)
import Enlil.Postgres.Hashes.Queries qualified as HashQ
import Enlil.Postgres.IDs
import Enlil.Postgres.Patches.Types
...
savePatch :: PatchHash -> PgPatch -> CodebaseM e PatchId
savePatch patchHash PatchFull.Patch {termEdits, typeEdits} = do
