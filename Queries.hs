...
-- >>> searchTokenToText (TypeMentionToken (NameSegment.unsafeParseText "Thing", fromJust $ SH.fromText "#2tWjVAuc7") (Just $ Occurrence 1))
-- ["mn,1,Thing","mh,1,#2tWjVAuc7"]
--
-- >>> searchTokenToText (TypeMentionToken (NameSegment.unsafeParseText "Thing", fromJust $ SH.fromText "#2tWjVAuc7") Nothing)
-- ["mn,r,Thing","mh,r,#2tWjVAuc7"]
--
-- >>> searchTokenToText (TypeVarToken (VarId 1) (Just $ Occurrence 1))
-- ["v,1,1"]
--
...
searchTokenToText = \case
  NameToken name -> [makeSearchToken nameType (reversedNameText name) Nothing]
  ...
  where
    ...
    makeSearchToken kind txt occTxt = do
      let occ = case occTxt of
            ...
            Nothing -> []
       in Text.intercalate "," $
            [kind] <> occ <> [Text.replace "," "" txt]

searchTokensToTsQuery :: Set (DefnSearchToken (Either NameSegment ShortHash)) -> Text
searchTokensToTsQuery tokens =
  Text.intercalate " & " $ searchTokenToText <$> Set.toList tokens
...
definitionSearch :: Maybe UserId -> Maybe DefnNameSearchFilter -> Limit -> Set (DefnSearchToken (Either NameSegment ShortHash)) -> Maybe Int -> Transaction e [(ProjectId, ReleaseId, Name, TermOrTypeSummary)]
    ...
    WITH matches_deduped_by_project(project_id, release_id, name, metadata) AS (
      SELECT DISTINCT ON (doc.project_id, doc.name) doc.project_id, doc.release_id, doc.name, doc.metadata FROM global_definition_search_docs doc
        ...
        WHERE
          ...
          plainto_tsquery(#{searchTokenText}) @@ doc.name
        AND (NOT p.private OR (#{mayCaller} IS NOT NULL AND EXISTS (SELECT FROM accessible_private_projects pp WHERE pp.user_id = #{mayCaller} AND pp.project_id = p.id)))
          ^{filters}
        ORDER BY doc.project_id, doc.name, r.major_version, r.minor_version, r.patch_version
    ),
